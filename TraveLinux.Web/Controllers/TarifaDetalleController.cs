...
{    
    public class TarifaDetalleController : BaseController
    {
        ...
        public ActionResult NuevaTarifaDetalle(int Proveedor, string TipoServicio,HttpPostedFileBase postedFile)
        {
                    ...
                    using (var package = new ExcelPackage(file.InputStream))
                    {
                        ...
                        if (TipoServicio == "TerAer")
                        {
                            ...
                            for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                            {
                                ...
                                user.FECHA_FINAL_S = workSheet.Cells[rowIterator, 4].Value == null ? string.Empty : workSheet.Cells[rowIterator, 4].Value.ToString();
                                user.TIPO_PERSONA = workSheet.Cells[rowIterator, 5].Value == null ? string.Empty : workSheet.Cells[rowIterator, 5].Value.ToString();
                                user.RANGO_PAX = workSheet.Cells[rowIterator, 6].Value == null ? 0 : Convert.ToInt32(workSheet.Cells[rowIterator, 6].Value.ToString());
                                user.PRECIO = workSheet.Cells[rowIterator, 7].Value == null ? 0 : Convert.ToDecimal(workSheet.Cells[rowIterator, 7].Value.ToString());                                
                                user.TIPO_ACOMODACION = workSheet.Cells[rowIterator, 8].Value == null ? string.Empty : workSheet.Cells[rowIterator, 8].Value.ToString();
                                user.TEMPORADA_S = workSheet.Cells[rowIterator, 9].Value == null ? string.Empty : workSheet.Cells[rowIterator, 9].Value.ToString();
                            ...
                            Fachada.Guardar_Carga_TerAer_Temporal(usersList);
                        }
            ...
            return RedirectToAction("NuevaTarifaDetalle", new RouteValueDictionary(new { controller = "TarifaDetalle", action = "NuevaTarifaDetalle", Proveedor = Proveedor }));
        }
        ...
        public ActionResult Carga_TerAer_Temporal()
        {
            var cuenta = Session["CUENTA"] as Cuenta;

            var vTarifaTerAer = Fachada.ObtenerTariTerAer();

            return Json(vTarifaTerAer);
        }
