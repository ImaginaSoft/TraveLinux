    ...
    public class AccesoDatos
    {
        ...
        private void GuardarTarifa_Detalle_Hoteles(Tarifa_Detalle eEntidad, int validado)
        {
            using (var connection = new OracleConnection(_connectionString))
            {
                ...
                command.Parameters.Add("P_TIPO_SERVICIO", OracleDbType.Varchar2, 20).Value = eEntidad.TIPO_SERVICIO;
                command.Parameters.Add("P_TIPO_PERSONA", OracleDbType.Varchar2, 20).Value = eEntidad.TIPO_PERSONA;
                command.Parameters.Add("P_SGL_ROOM", OracleDbType.Decimal,7).Value = eEntidad.SGL_ROOM;
                command.Parameters.Add("P_DWL_ROOM", OracleDbType.Decimal, 7).Value = eEntidad.DWL_ROOM;
                command.Parameters.Add("P_TPL_ROOM", OracleDbType.Decimal, 7).Value = eEntidad.TPL_ROOM;
                command.Parameters.Add("P_CDL_ROOM", OracleDbType.Decimal, 7).Value = eEntidad.CDL_ROOM;
                command.Parameters.Add("P_PERIODO", OracleDbType.Decimal, 7).Value = eEntidad.PERIODO;
                command.Parameters.Add("P_VALIDADO", OracleDbType.Int32).Value = validado;
                connection.Open();
                command.ExecuteNonQuery();
            }
        ...
        public void Copiar_Temporal_ServicioHotel(int iProveedor)
        {
            using (var connection = new OracleConnection(_connectionString))
            {
                var command = new OracleCommand();
                command.Connection = connection;
                command.CommandText = string.Concat(Globales_DAL.gs_PACKAGENAME, "SP_COPIAR_TARIFA_DETALLE_HOTEL");
                command.CommandType = CommandType.StoredProcedure;

                command.Parameters.Add("P_PROVEEDOR", OracleDbType.Int32).Value = iProveedor;
                connection.Open();
                command.ExecuteNonQuery();
            }
        }
        ...
        public Tarifa GuardarTarifaHTL(Tarifa eTarifa)
        {
            ...
            using (var connection = new OracleConnection(_connectionString))
            {
                ...
                command.Parameters.Add("P_SERVICIO", OracleDbType.Varchar2, 50).Value = eTarifa.SERVICIO;
                command.Parameters.Add("P_PROVEEDOR", OracleDbType.Varchar2, 50).Value = eTarifa.PROVEEDOR;
                command.Parameters.Add("P_ID_TIPO_HAB", OracleDbType.Varchar2, 50).Value = eTarifa.TIPO_HAB;
